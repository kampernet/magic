...
class Aspect {
	...
	public function compile() {
		...
		foreach($this->methodNames as $origMethodName) {
			...
			foreach($this->advice as $advice) {
				...
				if (!$advice instanceof AspectInterface) continue; // a tiny bit of security, as we're using eval, this might just help make sure ppl know what they're doing
	...
	private function getAfterIndex($methodName) {
		...
		if ($index !== false) {
			...
			if ($nextMethod === false) $nextMethod = count($this->classCodeArray) - 2; // -2 to make sure we get rid of class end brace
	...
	private function generateMethodSignature(ReflectionMethod $method) {
		...
		if ($haz_params) $signature = substr($signature, 0, -2);
	...
	private function generateMethodInvoking(ReflectionMethod $method) {
		...
		if ($haz_params) $invoke = substr($invoke, 0, -2);
	...
	private function applyBeforeAdvice(ReflectionClass $class, ReflectionMethod $method, ReflectionClass $adviceClass) {
		...
		foreach($adviceMethods as $adviceMethod) {
			...
			if ($method->getDeclaringClass()->getName() != $class->getName() &&
					!in_array($method->getName(), $this->adviceMethodNames)) { // and not already override generated
